...
function noteCreatedAtMs(note: ClientNote) {
  return new Date(note.created_at).getTime();
}

function sortNotesNewestFirst(notes: ClientNote[]) {
...
interface SavedClientNoteProps {
  note: ClientNote;
  onSave: (id: string, body: string) => void;
  onDelete: (id: string) => void;
}

function SavedClientNote({ note, onSave, onDelete }: SavedClientNoteProps) {
  ...
  return (
    <div className="flex items-start gap-4 py-3">
      ...
      <Button
        ...
        variant="ghost"
        size="sm"
        className="-mt-1 h-8 shrink-0 px-2 text-muted-foreground"
...
export function ClientNotesSection({ clientKey }: ClientNotesSectionProps) {
  const { clientNotes, addClientNote, updateClientNote, deleteClientNote } = useScheduling();
  ...
  const savedNotes = useMemo(
    () =>
      sortNotesNewestFirst(
        clientNotes.filter((n) => n.client_key.trim().toLowerCase() === normalizedKey),
      ),
    [clientNotes, normalizedKey],
  );
  ...
  const handleAdd = () => {
    ...
    setSaved(false);
    try {
      addClientNote(normalizedKey, trimmed);
      setDraft("");
      setSaved(true);
    } finally {
      setSaving(false);
      ...
      <CardContent className="space-y-6">
        {savedNotes.length > 0 && (
          <section className="space-y-1">
            <h3 className="mb-2 text-sm font-semibold">Saved notes</h3>
            <div className="divide-y divide-border/60">
              {visibleSavedNotes.map((note) => (
                <SavedClientNote
                  key={note.id}
                  note={note}
                  onSave={updateClientNote}
